...
Lifter.prototype.lift = function (failHard) {
	...
	// Scan recursively
	this._scan(lifted, this.folder, '', Boolean(failHard))
 ...
 * @private
 */
Lifter.prototype._scan = function (lifted, folder, name, failHard) {
	var that = this
...
Lifter.prototype._liftFile = function (lifted, action) {
	...
	var run = lifted._run(action.filters.concat(action.handler)).profile(action.profile)
	action.run = run.exec.bind(run)
}
...
Lifter.prototype._failFile = function (lifted, action, e) {
	// Save the error and create stub run()
	action.error = e
	var run = lifted._run(function () {
		throw e
