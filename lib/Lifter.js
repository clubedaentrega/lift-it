...
Lifter.prototype.lift = function (failHard) {
	...
	// Scan recursively
	this._scan(lifted, this.folder, '', Boolean(failHard), run, false)
...
Lifter.prototype.leanLift = function () {
	...
	this._scan(lifted, this.folder, '', true, null, true)
	return lifted
}
 ...
 * @private
 */
Lifter.prototype._scan = function (lifted, folder, name, failHard, run, lean) {
	var that = this
	fs.readdirSync(folder).forEach(function (each) {
		...
		if (stat.isDirectory()) {
			that._scan(lifted, subPath, subName, failHard, run, lean)
